...
| оглавление | из самой книги: её `toc` у `epub`, встроенный индекс у `mobi`, секции у `fb2`; текущая глава отмечена |
...
Оглавление берётся оттуда, где его ведёт сам файл, а не угадывается по
заголовкам: у `epub` это его `toc`, разложенный по файлам spine, у `mobi`
и `azw3` — встроенный индекс NCX, у `fb2` — секции. Обложка без названия,
сноски, пронумерованные «1», «2», «3», и безымянные куски отдельными главами
не становятся: они приклеиваются к соседям, а не плодят строки в оглавлении.

Книга разбирается **один раз, при загрузке**: библиотека только пополняется,
файл не меняется, и распаковывать zip на каждое открытие незачем. Разбор
версионируется — когда меняется парсер, книги перечитываются сами при первом
открытии, а браузер узнаёт об этом по `ETag` вместо повторной загрузки.
...
Что сделано, что запланировано и что **осознанно не делается** — в
[TODO.md](TODO.md); ближайшее большое там — озвучка книги голосом
(разбор рынка и конкурентов: [marketknows.md](marketknows.md)). Решения,
