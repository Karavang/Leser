...
Одна на все форматы — `epub`, `fb2` и `.rda` открываются одинаково, потому что
формат разбирает сервер, а не браузер.
...
| навигация | стрелки, пробел, Home/End, свайп, кнопки по краям, полоса по книге |
...
GET    /read/{id}.{ext}    книга для читалки: главы с разметкой
...
- **Разметка для читалки собирается сервером** из фиксированного набора тегов;
  ...
  с содержимым. Проверяется тестом `hostile_markup_is_neutralised`.
